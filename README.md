...
3. **Feature selection** (three passes, each shrinking `dataset.columns`):
   ...
   - Drop columns deemed unintuitive/unavailable to investors at loan-issue time (keeps an explicit `keep_list`). This also excludes `last_pymnt_amnt` — a post-origination field (the borrower's most recent payment on an already-funded loan) that doesn't exist at approval time, so keeping it would leak the outcome being predicted.
...
6. **Class balancing** — undersample both classes to 5,500 rows each (dataset is naturally ~79/21 imbalanced) and shuffle. This runs on the raw encoded features, before scaling/PCA.
7. **Train/validation split** — `train_test_split` with `test_size=0.2, random_state=7`, also on the raw encoded features (still before scaling/PCA).
8. **Dimensionality reduction** — impute missing values using the training split's mean only, then fit `StandardScaler` and `PCA(n_components=0.95, random_state=7)` on the training split only, and apply those same fitted transforms to the validation split, replacing the feature columns with the resulting principal components (`pc_1`, `pc_2`, …). Fitting scaling/PCA (or computing the imputation mean) on the full dataset before the split would leak validation-set statistics into the transform and inflate validation metrics — an earlier version of this notebook had that bug.
9. **Model comparison** — 10-fold CV (`scoring='roc_auc'`) across LR, LDA, KNN, CART, GaussianNB, MLPClassifier, AdaBoost, GradientBoosting, RandomForest, ExtraTrees. On the PCA-reduced features, Logistic Regression and LDA are statistically tied (ROC AUC ≈ 0.734, within one standard deviation of each other) — pick whichever model tops this comparison, since PCA's linear/decorrelated components can shift which model is best.
...
12. **Export artifacts** — `joblib.dump` the fitted model to `best_model.joblib`, the per-column `LabelEncoder`s to `label_encoders.joblib`, and the `StandardScaler`/`PCA` objects (fit on the training split only) to `pca_scaler.joblib`/`pca_transform.joblib`, so new raw data can be preprocessed identically (encode → scale → PCA transform) at inference time.

When modifying the pipeline, keep the encoders/scaler/PCA and the final model in sync: any change to `categorical_cols`, `keep_list`, or the PCA variance threshold upstream must be reflected in what's persisted in the corresponding `.joblib` file, since inference code outside this notebook would rely on all of them matching. If you swap in a different winning model at step 9–10, update step 11's feature-importance approach accordingly (`feature_importances_` vs `coef_`). Keep scaling/PCA/imputation fit on the training split only (step 8) — don't move them back before the split.
